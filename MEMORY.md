...
29. ℹ️ **Dikkat (potansiyel açık):** `PrivacyMasker.mask()/unmask()` şu an **no-op** (metni değiştirmiyor) — yani asıl müşteri akışında PII maskeleme fiilen kapalı. KVKK iddiası için regex tabanlı en azından telefon/e-posta/TC maskelemesi bağlanmalı (Enterprise NER opsiyonel).
